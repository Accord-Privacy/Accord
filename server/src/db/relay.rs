...
impl RelayDatabase {
    ...
    async fn run_migrations(&self) -> Result<()> {
        ...
        .await
        .ok();
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_device_keys_user ON device_keys (user_id)")
            .execute(&self.pool)
            .await
            .ok();
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_device_keys_fingerprint ON device_keys (device_fingerprint_hash)")
            .execute(&self.pool)
            .await
            .ok();
        sqlx::query("CREATE INDEX IF NOT EXISTS idx_device_registrations_fingerprint ON device_registrations (device_fingerprint_hash)")
            .execute(&self.pool)
            .await
            .ok();
    ...
    // ── Username & device key operations ──
    ...
    /// Set username on an existing user account
    pub async fn set_username(&self, user_id: Uuid, username: &str) -> Result<()> {
    ...
    pub async fn create_user_with_username(
        ...
        // the device key serves that purpose. Use a placeholder hash.
        let placeholder_pk = format!("v2-user-{}", user_id);
        let public_key_hash = super::compute_public_key_hash(&placeholder_pk);
    ...
    pub async fn get_device_keys_for_user(
        &self,
        user_id: Uuid,
    ) -> Result<Vec<super::super::models::DeviceKey>> {
        use super::super::models::DeviceKey;
